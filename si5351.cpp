...
uint8_t Si5351::si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	...
	return write(fd, buf, bytes+1);		// looks like retun value is not actually used in the main program, change this code later if necessary
}
...
uint8_t Si5351::si5351_read(uint8_t addr)
{
	...
	buf[0] = addr;
	write(fd, buf, 1);
	read(fd, &reg_val, 1);		// assume all read is always for a single byte
